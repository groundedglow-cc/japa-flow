...
10.1 如果一个大题在同一位置连续出现 2 个或 3 个以上例句，并且后续小题按编号区间分别套用这些例句，例如「例1 → 1-6」「例2 → 7-10」「例3 → 11-14」，也必须拆成多个 `PracticeItemGroup`。preview 必须展示为「例1 + 1-6 小题」「例2 + 7-10 小题」「例3 + 11-14 小题」，禁止展示成「例1、例2、例3 + 1-14 全部小题」。
...
- 如果例句里只有部分汉字、数字、金额、楼层或专有读法需要标音，优先把例句拆成 `beforeParts` / `after` 的多个 `RichText` 分片，只在需要标音的片段上填写 `kana`。不要为了给汉字补读音而对整句填写 `beforeKana` / `afterKana`，否则纯假名、片假名、助词和标点也会被错误标上 ruby。
- `beforeKana` / `afterKana` 只适合整行都确实需要按照教材标音展示的情况；普通练习例句默认使用分片 `RichText.kana`。
