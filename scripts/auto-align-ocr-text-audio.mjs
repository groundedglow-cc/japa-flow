...
if (!lessons.length || lessons.some((lesson) => !Number.isInteger(lesson) || lesson < 1 || lesson > 48)) {
  throw new Error("Usage: node scripts/auto-align-ocr-text-audio.mjs <lessonNo> [...lessonNo]");
}

const root = process.cwd();
const results = [];
for (const lesson of lessons) {
  const inputPath = join(root, "data", "ocr", `lesson${lesson}-text.json`);
  if (!existsSync(inputPath)) throw new Error(`Missing text OCR data: ${inputPath}`);
  results.push(await run(join(root, "scripts", "align-ocr-text-audio.mjs"), ["--lesson", String(lesson), "--auto-approve"]));
}
