...
import { handleAiApi } from "./server/ai/handlers.mjs";
...
async function handleApi(req, res, url) {
  try {
    // ============ AI ASSISTANT APIs ============
    // /api/ai/health  /api/ai/chat  /api/ai/explain  — handled in server/ai/handlers.mjs.
    if (url.pathname.startsWith("/api/ai/")) {
      if (await handleAiApi(req, res, url)) return true;
    }
